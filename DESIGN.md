...
- 正式ロゴ: `assets/img/LLF-img/LLF_2026.svg`
  ...
  - トップページのヒーロー（黒背景 + 星空）のように濃い背景に置く場合は、`filter: brightness(0) invert(1);` を img に当てて白抜き表示にする（`#logo img` を参照）。別途白抜き版のSVGファイルを用意する必要はない。
...
- ヘッダーのロゴは `LLF_2026.svg`、ナビゲーションリンクは `Home / About / NEWS / Schedule / History / BREMONS / ID / Documents` を維持する。
